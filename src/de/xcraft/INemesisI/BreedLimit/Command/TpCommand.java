...
public class TpCommand extends XcraftCommand {
	...
	public TpCommand() {
		super("breedlimit", "tp", "t.*", "<#>", "Teleports you to entry <#>",
	...
	public boolean execute(XcraftPluginManager manager, CommandSender sender, String[] args) {
		...
		if (entry < 0 || pmanager.scan.size() <= entry) {
			manager.plugin.messenger.sendInfo(player, ChatColor.RED + "This Entry does not exist!",
					true);
			return true;
		}
		String[] split = pmanager.scan.get(entry).split(":")[0].replace("Chunk(", "")
				.replace(")", "").split(", ");
